...
typedef struct {
    ...
    int action;         // Which action is being re-bound. Valid options are 0-11
    int rapid;          // Allows for rapid placement/removal of blocks. 1 = Remove, 2 = Replace
    int wait;           // Allows for pauses between placement of blocks
} Model;
...
void parse_command(const char *buffer, int forward) {
    ...
    // Displays all accepted arguments in the console
    else if (strcmp(buffer, "/set") == 0) {
        printf("Please enter the command as /set <integer>\n");
...
void on_mouse_button(GLFWwindow *window, int button, int action, int mods) {
    ...
    int exclusive =
        glfwGetInputMode(window, GLFW_CURSOR) == GLFW_CURSOR_DISABLED;
    // When the mouse button is released, stop placing/removing blocks
    if (action == GLFW_RELEASE) {
        g->rapid = 0;
        g->wait = 0;
        return;
...
void change_key(int key) {
    ...
    // Re-write control file to save changes
    save_controls();
}
...
void save_controls() {
    ...
    // Record all custom keys in order
    fprintf(f, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d", c->key_forward, c->key_backward, c->key_left, c->key_right, c->key_jump, c->key_fly, c->key_observe, c->key_observe_inset, c->key_item_next, c->key_item_prev, c->key_zoom, c->key_ortho);

    fclose(f);
}

/*
 * Loads controls from the generated text file
...
void load_controls_txt() {
    ...
    if (f == NULL || fgets(contents, 256, f) == NULL) {
        return;
    }
    ...
    // Go through each key, mapping every action in order
    while (token) {
        change_key(atoi(token));
        token = strtok(NULL, ",");
        g->action++;
    }
...
int main(int argc, char **argv) {
    ...
    while (running) {
        ...
        while (1) {
            ...
            if (g->rapid == 1 && (g->wait % 10) == 0) {
                on_right_click();
                g->wait++;
            }
            else if (g->rapid == 2 && (g->wait % 10) == 0) {
                on_left_click();
                g->wait++;
            }
            else if (g->rapid != 0) {
